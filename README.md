...
4. 重建需要更新的衍生指标，校验日期覆盖、指数、31个行业、融资数据、行业PB及其时点历史分位的完整性。
5. 校验通过后生成单页 `index.html`，其中包含行业PB横截面离散度，以及可切换31个申万一级行业的每日整体PB与时点历史分位曲线；随后提交并推送到 GitHub Pages。
...
- 行业PB分位采用时点口径：当日PB在该行业截至当日全部历史PB中的百分位，不使用未来数据；新增交易日会在每日更新时自动延长该序列。
